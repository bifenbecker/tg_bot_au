...
class BflContainer:
    bot: Bot
    current_state: Optional[BflState] = None
    data: dict
    ...
    def __init__(self, bot: Bot):
        ...
        self.bot.register_message_handler(self.set_region_name_handler,
                                          func=lambda
                                              message: self.current_state.name == BflState.SET_REGION_NAME.name)
        self.bot.register_message_handler(self.set_insolvent_situation_handler,
                                          func=lambda
                                              message: self.current_state.name == BflState.SET_INSOLVENT_SITUATION.name)
        self.bot.register_callback_handler(self.set_amount_expense_handler,
                                           func=lambda
                                               message: self.current_state.name == BflState.SET_AMOUNT_EXPENSE.name)
        self.bot.register_callback_handler(self.set_guarantees_handler,
                                           func=lambda
                                               message: self.current_state.name == BflState.SET_GUARANTEES.name)
        self.bot.register_callback_handler(self.set_experience_handler,
                                           func=lambda
                                               message: self.current_state.name == BflState.SET_EXPERIENCE.name)
        self.bot.register_message_handler(self.set_telephone_handler,
                                          func=lambda
                                              message: self.current_state.name == BflState.SET_TELEPHONE.name)

    def entry(self):
        self.set_state(next_state=BflState.SET_REGION_NAME)

    def set_region_name_handler(self, message: types.Message):
        logger.debug(f"STATE - {BflState.SET_REGION_NAME.name}. Message - {message.text}")
        self.data.update({
            "region_name": message.text
        })
        self.set_state(next_state=BflState.SET_INSOLVENT_SITUATION)

    def set_insolvent_situation_handler(self, message: types.Message):
        logger.debug(f"STATE - {BflState.SET_INSOLVENT_SITUATION.name}. Message - {message.text}")
        self.data.update({
            "insolvent_situation": message.text
        })
        self.set_state(next_state=BflState.SET_AMOUNT_EXPENSE)

    def set_amount_expense_handler(self, callback: types.CallbackQuery):
        logger.debug(f"STATE - {BflState.SET_AMOUNT_EXPENSE.name}. Message - {callback.data}")
        self.data.update({
            "amount_expense": callback.data
        })
        self.set_state(next_state=BflState.SET_GUARANTEES)

    def set_guarantees_handler(self, callback: types.CallbackQuery):
        logger.debug(f"STATE - {BflState.SET_GUARANTEES.name}. Message - {callback.data}")
        self.data.update({
            "guarantees": callback.data
        })
        self.set_state(next_state=BflState.SET_EXPERIENCE)

    def set_experience_handler(self, callback: types.CallbackQuery):
        logger.debug(f"STATE - {BflState.SET_EXPERIENCE.name}. Message - {callback.data}")
        self.data.update({
            "experience": callback.data
        })
        self.set_state(next_state=BflState.SET_TELEPHONE)

    def set_telephone_handler(self, message: types.Message):
        logger.debug(f"STATE - {BflState.SET_TELEPHONE.name}. Message - {message.text}")
        self.data.update({
    ...
    def set_state(self, next_state: BflState):
        ...
        state_messages = {
            BflState.SET_REGION_NAME.name: messages.BFL_SET_REGION_NAME,
            BflState.SET_INSOLVENT_SITUATION.name: messages.BFL_SET_INSOLVENT_SITUATION,
            BflState.SET_AMOUNT_EXPENSE.name: messages.BFL_SET_AMOUNT_EXPENSE,
            BflState.SET_GUARANTEES.name: messages.BFL_SET_GUARANTEES,
            BflState.SET_EXPERIENCE.name: messages.BFL_SET_EXPERIENCE,
            BflState.SET_TELEPHONE.name: messages.BFL_SET_TELEPHONE,
        }
        state_keyboard = {
            BflState.SET_REGION_NAME.name: None,
            BflState.SET_INSOLVENT_SITUATION.name: None,
            BflState.SET_AMOUNT_EXPENSE.name: types.InlineKeyboardMarkup().add(
            ...
            BflState.SET_GUARANTEES.name: types.InlineKeyboardMarkup().add(
            ...
            BflState.SET_EXPERIENCE.name: types.InlineKeyboardMarkup().add(
            ...
            BflState.SET_TELEPHONE.name: None,
        }
